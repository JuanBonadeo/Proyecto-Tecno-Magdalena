                ...
                <div className="controls">
                  ...
                  {prod.descuento > 0 ? (
                    <>
                      {prod.quantity >= 20 ? (
                        <>
                          <p className='price'>{formatearMoneda((prod.precio * prod.quantity) * 0.8)} </p>
                          <span className='discountedPrice'>Mayorista {calcularDescuento(prod.precio * prod.quantity * 0.8, prod.descuento)}</span>
                        </>
                      ) : (
                        <>
                    ...
                    <>
                      {prod.quantity >= 20 ? (
                        <>
                          <p className='price'>{formatearMoneda((prod.precio * prod.quantity) * 0.8)}</p>
                          <span className='discountedPrice'>Mayorista {calcularDescuento(prod.precio * prod.quantity * 0.8, 0)}</span>
                        </>
                      ) : (
                        <>
                          <p className='prices'>{formatearMoneda(prod.precio * prod.quantity)}</p>
                        </>
                      )}
                    </>
                  )}
